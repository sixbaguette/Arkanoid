...
public class PlayerScript : MonoBehaviour
{
    ...
    public float currentSpeed = 0f; // Vitesse actuelle
    private float targetSpeed = 0f; // Vitesse cible en fonction de la vitesse max

    private Rigidbody2D rb;

    [Range(0f, 1f)]
    public float boundaryPercentage = 1f;
    ...
    {
        Vector3 down = Camera.main.ViewportToWorldPoint(new Vector2(1, 0));
        Vector3 up = Camera.main.ViewportToWorldPoint(new Vector2(1, 1));
        Gizmos.color = Color.red;

        Gizmos.DrawLine(new Vector3(-boundary, -10, 0), new Vector3(-boundary, 10, 0));
        Gizmos.DrawLine(new Vector3(boundary, -10, 0), new Vector3(boundary, 10, 0));

        Gizmos.DrawLine(new Vector3(-10, down.y, 0), new Vector3(10, down.y, 0));
        Gizmos.DrawLine(new Vector3(-10, up.y, 0), new Vector3(10, up.y, 0));
    }
    #endregion
}